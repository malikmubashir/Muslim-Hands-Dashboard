  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          <ArrowLeft size={16} /> {t('td.allCauses')}
        </button>
        <div className="flex items-center gap-3">
          {onExtract && (
            ...
            >
              <Contact size={14} /> {t('td.dlThisCause')}
            </button>
          )}
          <span className="text-xs text-gray-500">
            {t('td.period')} : <span className="font-medium text-gray-700">{fmtMonth(range.start)} → {fmtMonth(range.end)}</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <KpiCard icon={Euro} label={t('kpi.totalRaised')} value={fmtEur(s.total)} accent={paletteAt(0)} />
        <KpiCard icon={Receipt} label={t('kpi.numDonations')} value={fmtNum(s.count)} accent={paletteAt(1)} />
        <KpiCard icon={TrendingUp} label={t('kpi.avgGift')} value={fmtEur2(s.count ? s.total / s.count : 0)} accent={paletteAt(2)} />
        <KpiCard icon={Percent} label={t('td.pctOfTotal')} value={fmtPct(shareOfTotal * 100)} accent={paletteAt(3)} />
        <KpiCard
          icon={CalendarCheck}
          label={t('td.bestMonth')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Stipulation donut */}
        <ChartCard title={t('td.stipulation')} sub={onExtract ? t('ov.clickSlice') : t('ov.stipSub')} exportName={`${slugBase}-stipulation`}>
          ...
          {onExtract && <CategoryDownloadBar label={t('ov.dlByStip')} items={s.byStipulation} onPick={(name) => onExtract({ cause: [theme], stip: [name] })} />}
        </ChartCard>
        ...
        >
          ...
          {onExtract && <CategoryDownloadBar label={t('ov.dlByPay')} items={payments} onPick={(name) => onExtract({ cause: [theme], pay: [name] })} />}
        </ChartCard>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Destinations */}
        <ChartCard title={t('td.destinations')} sub={onExtract ? t('ov.clickBar') : t('td.destSub')} exportName={`${slugBase}-destinations`}>
          ...
          {onExtract && <CategoryDownloadBar label={t('ov.dlByDest')} items={destinations} onPick={(name) => onExtract({ cause: [theme], dest: [name] })} />}
        </ChartCard>

        {/* Top 10 cities */}
        <ChartCard title={t('td.cities')} sub={onExtract ? t('ov.clickBar') : t('td.citiesSub')} exportName={`${slugBase}-villes`}>
          <ResponsiveContainer width="100%" height={340}>
          ...
          {onExtract && <CategoryDownloadBar label={t('td.dlByCity')} items={topCities} onPick={(name) => onExtract({ cause: [theme], city: name })} />}
        </ChartCard>
      </div>

      {/* Monthly area — full width */}
      <ChartCard title={t('ov.evoTitle')} sub={t('ov.evoSub')} exportName={`${slugBase}-evolution`}>
      ...
      {/* Top 10 départements */}
      <ChartCard title={t('td.depts')} sub={onExtract ? t('ov.clickBar') : t('td.deptsSub')} exportName={`${slugBase}-departements`}>
        <ResponsiveContainer width="100%" height={320}>
        ...
        {onExtract && <CategoryDownloadBar label={t('td.dlByDept')} items={topDepts.map((d) => ({ name: d.code }))} onPick={(code) => onExtract({ cause: [theme], dept: code })} />}
      </ChartCard>
      </>
      )}
    </div>
